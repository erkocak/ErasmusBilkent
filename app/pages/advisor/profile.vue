...
let user: Ref<Advisor | undefined> = ref()
...
onMounted(async () => {
    if (auth.value) {
        ...
        user.value = (await getProfileById(id, role)) as Advisor
    }
    else {
...
<template>
    <NavbarAdvisor active="Profile"/>
    <profileCard :user="user" v-if="user != undefined"/>
