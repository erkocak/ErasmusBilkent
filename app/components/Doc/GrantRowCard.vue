...
let downloading_uploaded = ref(false)
...
async function handleFileUpload() {
    ...
    if (uploaded_doc.value) {
        ...
        const docs: Document[] = await getDocumentsByOwnerAndType(props.gd_row.user_id as string, "ega")
        ...
        if (doc.length != 0) {
            ...
            const doc_status: boolean = await updateDocument(doc_id, props.gd_row.user_id as string, "ega", "Grant Agreement", "pending")
            doc_status_g = doc_status

        }
        else {
            console.log("Inserting doc")
            const doc_status: boolean = await insertDocument(props.gd_row.user_id as string, "ega", "Grant Agreement ")
            doc_status_g = doc_status
        }

        if (doc_status_g) {
            const file_status: boolean = await uploadDocument(uploaded_doc.value as File, props.gd_row.user_id as string, "ega", "learningagreement")
            if (file_status) {
...
onMounted(async () => {
    ...
    grant.value = props.gd_row.documents.has("ega-s")
...
<template>
    <div class="container">
        <div class="row">
            ...
            <div class="col d-flex align-items-center justify-content-center">
                <button class="btn btn-primary" :class="{ disabled: !grant }" @click="downloadUploadedFile('ega-s')">
