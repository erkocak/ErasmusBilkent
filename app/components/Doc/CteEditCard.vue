...
const doc_signed: Ref<Document | undefined> = ref()
...
async function fetchSignedDoc() {
    loading.value = true
    const docs: Document[] = await getDocumentsByOwnerAndType(user.value?.id as string, props.cardCode + "-s")
    console.log("DOCSSS:", docs)
    if (docs.length > 0) {
        let documents= Document.instantiateDocumentArray(docs)
        doc_signed.value = documents[0]
    }
    loading.value = false
}
...
async function downloadSignedDocument() {
    ...
    if (doc.value && user.value){
        ...
        await downloadFile("learningagreement", user.value.id + "/" + doc.value.getType() + "-s")
...
async function handleFileUpload() {
    ...
    if (fileUploaded.value) {
        ...
        const doc: Document[] = await getDocumentsByOwnerAndType(user.value?.id as string, props.cardCode)
    ...
    <div class="bg-light rounded border border-dark">
        ...
        <div class="container text-center">
            <div v-if="!loading" class="row d-flex justify-content-center align-items-center" style="min-height: 20vh">
                ...
                <div v-if="doc_signed != undefined" class="col d-flex justify-content-center align-items-center flex-column ">
